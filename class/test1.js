...
const { encryptPassword } = require('./1118/util');
const { body, validationResult } = require('express-validator');

const { User }=require('./1118/user');
const { Task }=require('./1118/task');
...
// 내 할일 중 하나
app.get("/tasks/:id", authentication, taskAuthentication, async (req, res) => {
  res.send(req.task);
});
...
// 내 할일인지 확인하고 수정
app.put("/tasks/:id", authentication, taskAuthentication, async (req, res) => {
  const task = req.task;
  const { title, description, dueDate } = req.body;
  task.title = title;
